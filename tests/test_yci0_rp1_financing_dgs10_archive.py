...
class FinancingDGS10ArchiveTests(unittest.TestCase):
    def test_extracts_frozen_complete_month_end_points(self):
        rows = module.extract_month_end_points(SAMPLE)
        self.assertEqual([(r['month_end'], r['observation_date'], r['value_pct']) for r in rows], [
    ...
    def test_missing_required_month_fails_closed(self):
        with self.assertRaisesRegex(RuntimeError, 'missing complete-month observation'):
