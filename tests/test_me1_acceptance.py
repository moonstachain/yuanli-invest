...
class ME1AcceptanceTests(unittest.TestCase):
    def test_human_acceptance_receipt_matches_state(self):
        ...
        self.assertEqual(receipt["reviewed_ci"]["conclusion"], "success")
        self.assertEqual(receipt["formal_review"], "13/13 PASS")
        self.assertEqual(state["human_acceptance"], receipt["decision"])
        self.assertEqual(state["human_acceptance_reviewed_head_sha"], receipt["reviewed_head_sha"])
        self.assertEqual(state["status"], "human_accepted_pending_post_acceptance_ci")
        self.assertEqual(state["next_gate"], "ME1_POST_ACCEPTANCE_CI")
