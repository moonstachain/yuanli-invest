...
# These are forbidden as object property keys. Skills may enumerate the same
# strings as values under prohibited_outputs; that is the intended guardrail.
FORBIDDEN_PROPERTY_KEYS = {
    "target_price", "buy_signal", "sell_signal", "recommended_weight",
    "target_weight", "position_size", "broker_action", "live_execution",
    "pnx_score", "force_score",
}
...
def validate_pack(name: str, schema_name: str, object_type: str, expected_count: int):
    ...
    id_key = {
        "TheoryObject": "theory_id", "HypothesisObject": "hypothesis_id",
        "FactorObject": "factor_id", "AlgorithmObject": "algorithm_id",
        "BenchmarkObject": "benchmark_id", "SkillContract": "skill_id",
        "CanonicalDataField": "field_id", "ResearchCapability": "capability_id",
    }[object_type]
    ...
    for obj in pack["objects"]:
        ...
        seen.add(identity)
        forbidden = FORBIDDEN_PROPERTY_KEYS & set(property_keys(obj))
        assert not forbidden, f"forbidden action/scalar property keys in {identity}: {sorted(forbidden)}"
...
def main() -> None:
    ...
    for alg in objects["algorithms"]:
        ...
        if alg["domain"] == "XA":
            assert alg["causal_claim_status"] == "predictive", "R2 Xa cannot overclaim causality"

    theory = {obj["theory_id"]: obj for obj in objects["theories"]}
    assert theory["THEORY-SHILLER-2017-NARRATIVE-ECONOMICS"]["evidence_status"] == "primary_source_verified"
    xu = theory["THEORY-XU-2026-CAUSAL-EXTREME-SIGNALS"]
    assert xu["source_class"] == "practitioner_claim" and xu["evidence_status"] == "practitioner_claim"
    ...
    for cap in objects["capabilities"]:
        if xu["theory_id"] in cap["theory_ids"]:
    ...
    for skill in objects["skills"]:
        assert skill["runtime_class"] == "generic_agent", "R3 owns Wind/Codex-specific interfaces"
    ...
    assert r2_state["capability_count"] == 12 and r2_state["registry_entry_count"] == 99
    ...
    docs_text = "\n".join((R2 / name).read_text(encoding="utf-8") for name in (
        "R2-PNXS-GOLD-CAPABILITY-PACK-v0.1.md",
        "R2-SOURCE-BOUNDARY-MAP-v0.1.md",
        "R2-HUMAN-REVIEW-CARD-v0.1.md",
    ))
